...
```bash
# Pull the image
docker pull dhub.ksr.la/wxocr

# Run the container
docker run -d -p 5000:5000 --name wechat-ocr-api dhub.ksr.la/wxocr
```
...
OR post file to `/file-ocr`

```bash
curl --location 'http://localhost:5000/file-ocr' --form 'image=@"/tmp/file.png"' -s | awk -F'\t\t' '{print $1}'
```
